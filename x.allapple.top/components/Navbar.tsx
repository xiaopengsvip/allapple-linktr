...
import { Search, Moon, Sun, Menu, X, Globe, ChevronRight } from 'lucide-react';
import { NAV_LINKS, ASSETS, STYLES } from '../constants';
...
const Navbar: React.FC<NavbarProps> = ({ toggleDarkMode, isDarkMode, lang, toggleLanguage }) => {
  ...
  const labels: Record<string, { zh: string; en: string }> = {
    ...
    dashboard: { zh: '指挥中心', en: 'Command' }, // Added
    customizer: { zh: '定制', en: 'Design' },
    features: { zh: '功能', en: 'Features' },
    specs: { zh: '规格', en: 'Specs' },
        ...
        <ul className="hidden md:flex gap-1 items-center bg-black/20 backdrop-blur-md rounded-full px-4 py-1.5 border border-white/5">
          {NAV_LINKS.map((link) => (
            <li key={link.key}>
              <a 
                href={link.href} 
                className="px-3 lg:px-4 py-2 text-xs lg:text-sm font-medium text-gray-300 hover:text-white rounded-full hover:bg-white/10 transition-all relative group uppercase tracking-wide"
              >
                {labels[link.key] ? labels[link.key][lang] : link.key}
              </a>
            </li>
          ))}
        </ul>
          ...
          <div className="flex flex-col items-center justify-start gap-3 w-full max-w-sm mx-auto flex-1">
            {NAV_LINKS.map((link, idx) => (
              <a 
                key={link.key}
                href={link.href} 
                className="group relative w-full py-4 px-6 rounded-2xl border border-white/5 bg-white/5 overflow-hidden transition-all duration-300 hover:border-accent/50 hover:shadow-[0_0_20px_rgba(0,113,227,0.3)] animate-slide-right opacity-0 flex items-center justify-between active:scale-[0.98]"
              ...
              >
                ...
                <div className="relative z-10 flex items-center justify-between w-full">
                  <span className="text-lg font-bold font-orbitron tracking-widest text-gray-200 group-hover:text-white transition-colors">
                    {labels[link.key] ? labels[link.key][lang] : link.key}
                  </span>
