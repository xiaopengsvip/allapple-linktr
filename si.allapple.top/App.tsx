...
import { Instagram, MapPin, Mail, Share2, Twitter, Play, Pause } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    if (!el) return;
    const update = () => setIsBgPlaying(!el.paused && !el.ended);
  ...
  return (
    <div className={`min-h-screen font-sans ${isDarkMode ? 'dark' : ''}`}>
      ...
      <div className="fixed top-0 left-0 w-full h-full -z-20 overflow-hidden">
        ...
        {/* 如果自动播放被阻止或视频未播放，显示可见的播放按钮以便用户启动并解除静音 */}
        {!isBgPlaying && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-auto">
            <button
              onClick={() => {
                ...
                if (!v) return;
                // 尝试解除静音并播放（用户交互后浏览器允许解除静音）
                v.muted = false;
                v.play().catch(() => {
                  // 若播放仍被阻止，仅设置播放状态（用户可再试）
                });
              }}
              className="bg-white/10 hover:bg-white/20 text-white p-4 rounded-full backdrop-blur-md border border-white/10 shadow-lg"
      ...
      {/* 噪点与渐变覆盖层（调亮以减少灰暗感） */}
      <div className="fixed top-0 left-0 w-full h-full -z-10 bg-black/10 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-transparent to-black/10 pointer-events-none"></div>
      ...
      <main className="relative z-10 pb-32 md:pb-32 overflow-hidden">
        ...
        <Section id="video" className="w-full max-w-6xl mx-auto px-4 md:px-8 py-16 md:py-24">
          <div className={`${STYLES.glassPanel} rounded-2xl md:rounded-3xl p-6 md:p-12 transition-transform duration-500 hover:scale-[1.01]`}>
            ...
            <div className="relative pb-[56.25%] h-0 overflow-hidden rounded-xl md:rounded-2xl bg-black/50 shadow-lg border border-white/10 mb-6 md:mb-8 group">
              <video controls playsInline className="absolute top-0 left-0 w-full h-full object-cover" poster={ASSETS.introVideoCover}>
                <source src={ASSETS.bgVideo} type="video/mp4" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              {BLOG_POSTS.map((post, idx) => (
                <div 
                  ...
                  className="bg-gradient-to-br from-white/5 to-transparent p-6 md:p-10 rounded-2xl border border-white/10 hover:border-accent/50 hover:shadow-lg transition-all cursor-pointer group relative overflow-hidden"
                >
      ...
      {showQR && (
        ...
        >
          <div className={`${STYLES.glassPanel} p-8 md:p-10 rounded-3xl text-center flex flex-col items-center shadow-lg border-white/20 w-full max-w-sm`} onClick={e => e.stopPropagation()}>
