  ...
  return (
    <nav className={`fixed w-full z-[100] transition-all duration-500 border-b top-9 ${
      isScrolled || isMenuOpen
        ? `${STYLES.glassFloating} py-2 md:py-3 shadow-[0_12px_36px_0_rgba(0,0,0,0.1)] border-white/10` 
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-6 flex justify-between items-center relative">
        ...
        >
          <img src={ASSETS.logo} alt="Logo" className="h-7 w-7 md:h-9 md:w-9 rounded-xl transition-transform duration-500 group-hover:rotate-12 ring-1 ring-white/20 shadow-[0_6px_16px_0_rgba(0,0,0,0.08)]" />
          <span className="text-lg md:text-xl font-bold tracking-tight font-orbitron group-hover:text-accent transition-colors">allapple.top</span>
        </div>
      ...
      {/* 移动端全屏菜单 */}
      {isMenuOpen && (
        <div className="md:hidden fixed inset-0 z-[90] bg-black/10 backdrop-blur-3xl flex flex-col pt-24 pb-10 px-6 animate-fade-in overflow-y-auto h-[100dvh]">
          {/* 背景装饰元素 */}
          <div className="absolute top-0 right-0 w-[50vw] h-[50vw] bg-accent/10 rounded-full blur-[80px] pointer-events-none animate-pulse-slow"></div>
          ...
          <div className="flex flex-col items-center justify-start gap-3 w-full max-w-sm mx-auto flex-1">
            {NAV_LINKS.map((link, idx) => (
              <a 
                ...
                href={link.href} 
                onClick={(e) => handleNavClick(e, link.key, link.isExternal)}
                className={`group relative w-full py-4 px-6 rounded-2xl border bg-white/5 overflow-hidden transition-all duration-300 hover:shadow-[0_0_20px_rgba(0,113,227,0.1)] animate-slide-right opacity-0 flex items-center justify-between active:scale-[0.98]
