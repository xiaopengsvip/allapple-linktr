...
// 动态导入 Google GenAI SDK（仅在可用且有 API Key 时使用），避免在客户端构建期间立即引入可能导致打包/运行时错误的 Node 专用依赖。
...
const AiAssistant: React.FC<AiAssistantProps> = ({ isOpen, onClose, lang }) => {
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      ...
      try {
        ...
        const responseText = (result && (result as any).text) || (result as any).response || '';
        ...
        // 如果动态导入或调用 SDK 失败，进入下面的全局 catch 处理，记录错误以便排查
        console.error('GenAI dynamic import / request failed:', err);
        throw err;
      }
  ...
  return (
    <>
      {/* 聊天界面模态框 */}
      {isOpen && (
        <div className={`fixed bottom-24 left-4 right-4 md:left-auto md:right-8 md:w-[380px] z-[100] h-[500px] max-h-[70vh] flex flex-col ${STYLES.glassModal} rounded-2xl animate-fade-in overflow-hidden shadow-lg border border-white/20`}>
          {/* 标题栏 */}
