   ...
   const getPath = () => {
      ...
      const path = points
         .map((p, i) => {
            ...
            const y = +(100 - p).toFixed(2); // 翻转 Y 轴
            return `${i === 0 ? 'M' : 'L'} ${x} ${y}`;
         })
         .join(' ');
      return path;
                   ...
                   <defs>
                      <linearGradient id="gradient" x1="0%" y1="0%" x2="0%" y2="100%">
                         ...
                         <stop offset="100%" stopColor="#0071e3" stopOpacity="0" />
                            ...
                            {/* 只在有有效路径时渲染填充路径，确保路径以 moveto 开始 */}
                            {(() => {
                               const path = getPath().trim();
                               if (!path) return null;
                               const fillPath = `${path} L 100 100 H 0 Z`;
                               return (
                                  <>
                                     <path d={fillPath} fill="url(#gradient)" className="transition-all duration-500 ease-linear" />
                                     <path d={path} fill="none" stroke="#0071e3" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="transition-all duration-500 ease-linear" />
                                  </>
                               );
                            })()}
